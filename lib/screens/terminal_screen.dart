...
class _ConsoleScreenState extends State<ConsoleScreen> {
  ...
  void initState() {
    ...
    _append("Type 'help' for commands.");
    _loadCloudPref();
  ...
  Future<void> _runCommand(String cmd) async {
    ...
    if (lower == "help") {
      _append("Commands:");
      _append("  smart            run smart scan");
      _append("  smart /c         smart scan with cloud");
      _append("  rapid            run rapid scan");
      _append("  rapid /c         rapid scan with cloud");
      _append("  single           pick and scan one file");
      _append("  single /c        single scan with cloud");
      _append("  check [url]      check domain against threat database");
      _append("  vpn on           enable VPN");
      _append("  vpn off          disable VPN");
      _append("  clear            clear console");
      return;
    }
    ...
    if (lower == "smart" || lower == "rapid" || lower == "single") {
      ...
      if (lower == "smart") {
        _append(useCloud ? "[SCAN] Smart scan with cloud" : "[SCAN] Smart scan");
        await _runSmartScan(useCloud);
        return;
      }

      if (lower == "rapid") {
        _append(useCloud ? "[SCAN] Rapid scan with cloud" : "[SCAN] Rapid scan");
        await _runRapidScan(useCloud);
        return;
      }

      if (lower == "single") {
        _append(useCloud ? "[SCAN] Single file scan with cloud" : "[SCAN] Single file scan");
  ...
  Future<void> _scanFiles(List<String> files, bool useCloud) async {
    ...
    if (useCloud && _cloudScanner != null) {
      _append("[STAGE 1] Computing file hashes...");
      ...
      for (final path in files) {
        try {
          ...
          fileHashes[path] = {'md5': md5h, 'sha': sha};
          allHashes.add(md5h);
          allHashes.add(sha);
    ...
    _append(useCloud ? "[STAGE 3] Local scanning files..." : "[STAGE 1] Local scanning files...");

    for (final path in files) {
      final name = path.split('/').last;
      bool infectedFlag = false;

      if (useCloud && fileHashes.containsKey(path) && cloudDetected.isNotEmpty) {
  ...
  bool _isAllowedFile(String ext, int size) {
    const allowed = ['apk', 'zip', 'pdf', 'txt', 'md', 'exe'];
