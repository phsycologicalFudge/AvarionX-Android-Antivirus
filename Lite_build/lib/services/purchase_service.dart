...
class PurchaseService {
  ...
  static Future<bool> cachedHasPro() async {
    final prefs = await SharedPreferences.getInstance();

    final localPro = prefs.getBool(_kIsPro) ?? false;
    final permanentPro = prefs.getBool(_kPermanentPro) ?? false;
    final serverPro = (prefs.getBool(_kServerSessionSignedIn) ?? false) &&
        (prefs.getBool(_kServerSessionPro) ?? false);

    _isPro = localPro || permanentPro;
    _permanentPro = permanentPro;
    _serverSessionPro = serverPro;

    return _isPro || _serverSessionPro;
  }
